...
impl NormalData {
    ...
    #[staticmethod]
    #[inline]
    pub fn wrapped_list_of_const_data(list_constdata: Vec<Option<ConstData>>, repeat_time: usize) -> NormalData {
        ...
        for i in 0..n {
            if let Some(constdata) = &list_constdata[i] {
                ...
                let error_data = add_errors(&Array1::from_elem(repeat_time, mean), std * 5.).unwrap();
...
impl NormalData {
    pub fn new(data: Array2::<f64>) -> Self {
        ...
        for x in 0..n {
            if data.column(x).iter().any(|&a| a.is_nan() || a.is_infinite()) {
                badpts.insert(x);
                data.column_mut(x).fill(NAN);
            }
        }
        _gen_domain(n, &badpts).iter().filter(|(x, y)| y - x < 3).for_each(|(x, y)| {
    ...
    #[inline]
    pub fn bds_threshold (&self) -> f64 { 100000000. }
...
impl NormalData {
        ...
        for (x, y) in self.gen_domain() {
            ...
            if self.is_conserved_slice(x, y) {
                ...
                if error.is_finite() {
                    ...
                    data.slice_mut(s![.., x..y]).mapv_inplace(|_| normal.sample(&mut rng));
                } else {
                    data.slice_mut(s![.., x..y]).fill(NAN);
                }
            } else {
    ...
    pub fn to_const_data(&self) -> ConstData {
        ...
        let (mean, std) = estimate_const(&data.mean_axis(ndarray::Axis(0)).unwrap(), &data.std_axis(ndarray::Axis(0), 0.0));
    ...
    pub fn calc_mean(&self) -> Option<ConstData> {
        ...
        let (mean, std) = calc_mean_error(&data.mean_axis(ndarray::Axis(0)).unwrap(), &data.std_axis(ndarray::Axis(0), 0.0));
...
pub fn calc_mean_error(value: &Array1<f64>, std: &Array1<f64>) -> (f64, f64) {
    (value.mean().unwrap(), std.mean().unwrap() / (value.len() as f64).sqrt())
}
