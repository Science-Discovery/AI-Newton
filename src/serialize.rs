...
use serde::{Deserialize, Serialize, Serializer, Deserializer, ser::SerializeStruct, de::MapAccess};
use crate::{language::{AtomExp, Concept, Proposition}, experiments::{DoExpType, ExpStructure, Objstructure, Parastructure, ObjType}, knowledge::Knowledge};
            ...
            fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error>
                ...
                while let Some(key) = map.next_key()? {
                    match key {
                        ...
                        _ => {
                            return Err(serde::de::Error::unknown_field(key, &["expconfig", "do_experiment"]));
                        }
                    }
                }
                let expconfig = expconfig.ok_or_else(|| serde::de::Error::missing_field("expconfig"))?;
                let do_experiment = do_experiment.ok_or_else(|| serde::de::Error::missing_field("do_experiment"))?;
                Ok(ExpStructure::new(expconfig, do_experiment))
            }
        }
        deserializer.deserialize_struct("ExpStructure", &["expconfig", "do_experiment"], ExpStructureVisitor)
    }
}
