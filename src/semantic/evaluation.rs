...
use crate::expdata::{ConstData, Diff, ExpData};
...
pub fn _concept_apply_ids(
    ...
    match con {
        ...
        _ => match context {
            None => Err(format!(
                "exp_name is None in concept_apply_ids({}, {:?})",
                concept, ids
            )),
            Some(context) => {
                ...
                match res.len() {
                    ...
                    1 => Ok(res[0].clone()),
                    _ => Err(format!(
                        "More than one result in concept_apply_ids({}, {:?}",
                        concept, ids
                    )),
                }
            }
...
impl Knowledge {
    pub fn _get_expstructure(
        &self,
        expconfig: &IExpConfig,
        objsettings: Vec<Objstructure>,
    ) -> Result<ExpStructure, String> {
        match expconfig {
            IExpConfig::From { name } => {
                if objsettings.len() != 0 {
                    return Err(format!(
                        "length of objsettings does not match with {}",
                        expconfig
                    ));
                }
            ...
            IExpConfig::Mk {
                ...
                if obj.obj_type.to_string() != *objtype {
                    return Err(format!(
                        "objtype {} in objsettings does not match with {}",
                        obj.obj_type, expconfig
                    ));
                }
                ...
                match sexp {
                    SExp::Mk { expconfig, exp } => {
                        ...
                        loop {
                            ...
                            if total_time > 5 {
                                return Err(format!(
                                    "failed to evaluate intrinsic concept {}",
                                    intrinsic
                                ));
                            }
                            let mut context =
                                self._get_expstructure(expconfig, objsettings.clone())?;
                            let exp = exp.as_ref();
    ...
    pub fn _eval_prop(
        ...
        match prop {
            ...
            Proposition::IsConserved { exp } => Ok(self._eval(&exp, context)?.is_conserved()),
            Proposition::IsZero { exp } => Ok(self._eval(&exp, context)?.is_zero()),
            _ => unimplemented!(),
        }
    }
    ...
    fn __eval(&self, exp0: &Exp, context: &mut ExpStructure) -> Result<ExpData, String> {
        ...
        match exp0 {
            ...
            Exp::Number { num } => Ok(ExpData::from_exact_const(*num)),
            Exp::Atom { atom } => {
                let atom = atom.as_ref();
                // println!("atom = {}", atom);
                    ...
                    Err(_) => {
                        ...
                        match expr {
                            Expression::Intrinsic { intrinsic } => {
                                ...
                                context
                                    .get_mut_expdata()
                                    .set_data(atom.clone(), expdata.clone());
                                Ok(expdata)
                            }
                            Expression::Concept { concept } => {
                                if concept.atomexp_name() == Some(atom.get_name()) {
            ...
            Exp::UnaryExp {
                op: UnaryOp::Neg,
                ref exp,
            } => Ok(-self.__eval(&*exp, context)?),
            Exp::UnaryExp {
                op: UnaryOp::Diff,
                ref exp,
            } => Ok(self.__eval(&*exp, context)?.diff_tau()),
            Exp::BinaryExp {
                op,
                ref left,
                ref right,
                ...
                op,
                self.__eval(&*left, context)?,
                self.__eval(&*right, context)?,
            )),
            Exp::DiffExp {
                ref left,
                ref right,
                ord,
            } => Ok((&self.__eval(&*left, context)?)
                .diff_n(&self.__eval(&*right, context)?, *ord as usize)),
    ...
    #[inline]
    pub fn _raw_definition_concept(
        &self,
        concept: &Concept,
        context: Option<&ExpStructure>,
    ) -> Concept {
        match concept {
            Concept::Mk0 { exp } => Concept::Mk0 {
                exp: Box::new(self._raw_definition_exp(exp, context).doit()),
            },
            Concept::Mksucc {
                objtype,
                concept,
                ...
                concept: Box::new(self._raw_definition_concept(concept, context)),
                id: *id,
            },
