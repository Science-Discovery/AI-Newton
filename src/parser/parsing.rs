...
use crate::experiments::{Objstructure, Parastructure};

impl FromStr for Objstructure {
    type Err = String;
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        expr::ObjstructureParser::new()
            .parse(s)
...
pub fn parse_knowledge(
    ...
    (
        Vec<(String, Objstructure)>,
        Vec<(String, Expression)>,
        Vec<(String, Proposition)>,
    ),
    String,
> {
    expr::KnowledgeParser::new()
        .parse(input)
        .map_err(|e| e.to_string())
}
...
pub fn parse_sympy_to_exp(input: &str) -> PyResult<Exp> {
    sympy::ExpParser::new()
        .parse(input)
        .map(|e| *e)
        .map_err(|e| PyValueError::new_err(input.to_string() + &e.to_string()))
}

use crate::language::ast::{
    AtomExp, Concept, Exp, Expression, IExpConfig, Intrinsic, Proposition, SExp,
};
...
impl Knowledge {
    pub fn _parse_atomexp_to_sympy_str(&self, input: &AtomExp, argument: String) -> String {
        ...
        let not_with_argument = self
            ._made_of_obj_attr(&Exp::Atom {
                atom: Box::new(input.clone()),
            })
            .unwrap();
    ...
    pub fn _parse_exp_to_sympy_str(&self, input: &Exp, argument: String) -> String {
        match input {
            ...
            Exp::UnaryExp { op, exp } => match op {
                UnaryOp::Neg => {
                    format!("(-{})", self.parse_exp_to_sympy_str(exp.as_ref(), argument))
                }
                UnaryOp::Diff => {
            ...
            Exp::DiffExp { left, right, ord } => {
                ...
                if right == argument {
                    return format!("Derivative({}, {}, {})", left, right, *ord);
                }
