...
pub fn builtin_motiony(t_end: f64, t_num: usize, error: f64, exp_config: &ExpConfig) -> DataStructOfDoExperiment {
    ...
    data_struct.add_data((DATA::cy(), vec![r!("Slope")]), &add_errors(&Array1::from_elem(t_num, theta.sin()), error).unwrap());
...
pub fn struct_motiony() -> ExpStructure {
    ...
    exp_config.register_geometry_info(exp_config.gen_prop(r!("cy[Slope] * posy[MPa] + cz[Slope] * posz[MPa] is conserved")));
