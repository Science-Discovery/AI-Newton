...
pub fn builtin_stringmotion0(t_end: f64, t_num: usize, error: f64, exp_config: &ExpConfig) -> DataStructOfDoExperiment {
    ...
    data_struct.add_data((DATA::length(), vec![r!("SPa")]), &add_errors(&Array1::from_elem(t_num, spa_length_value), error).unwrap());
    data_struct
}
...
pub fn struct_stringmotion0() -> ExpStructure {
    ...
    let do_experiment: DoExpType = DoExpType::new(
        r!("<builtin_stringmotion0>"),
        builtin_stringmotion0
    );
