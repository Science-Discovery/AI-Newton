...
pub fn builtin_oscillation(
    ...
    let length = if x2 > x1 {
        sp2_length_value
            + (v2 / omega) * (omega * &t).mapv(|x| x.sin())
            + (x2 - x1 - sp2_length_value) * (omega * &t).mapv(|x| x.cos())
    } else {
        -sp2_length_value
            + (v2 / omega) * (omega * &t).mapv(|x| x.sin())
            + (x2 - x1 + sp2_length_value) * (omega * &t).mapv(|x| x.cos())
...
pub fn struct_oscillation() -> ExpStructure {
    ...
    let do_experiment: DoExpType = DoExpType::new(r!("<builtin_oscillation>"), builtin_oscillation);
    ...
    exp_config
        .register_geometry_info(exp_config.gen_prop(r!("length[SPb] - posx[MPa] is conserved")));
