...
pub fn builtin_oscillation(t_end: f64, t_num: usize, error: f64, exp_config: &ExpConfig) -> DataStructOfDoExperiment {
    ...
    let length = if x2 > x1 {
        sp2_length_value + (v2 / omega) * (omega * &t).mapv(|x| x.sin()) + (x2 - x1 - sp2_length_value) * (omega * &t).mapv(|x| x.cos())
    } else {
        - sp2_length_value + (v2 / omega) * (omega * &t).mapv(|x| x.sin()) + (x2 - x1 + sp2_length_value) * (omega * &t).mapv(|x| x.cos())
    };
    ...
    data_struct.add_data((DATA::posx(), vec![r!("MPa")]), &add_errors(&sp2_r, error).unwrap());
...
pub fn struct_oscillation() -> ExpStructure {
    ...
    exp_config.register_geometry_info(exp_config.gen_prop(r!("length[SPb] - posx[MPa] is conserved")));
