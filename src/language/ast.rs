...
impl Proposition {
    ...
    fn __neg__(&self) -> Self {
        match self {
            Proposition::Conserved {concept} => Proposition::Conserved {concept: Box::new(-concept.as_ref())},
            Proposition::Zero {concept} => Proposition::Zero {concept: Box::new(-concept.as_ref())},
            Proposition::IsConserved {exp} => Proposition::IsConserved {exp: Box::new(-exp.as_ref())},
            Proposition::IsZero {exp} => Proposition::IsZero {exp: Box::new(-exp.as_ref())},
...
pub enum Exp {
    ...
    Partial {left: Box<Exp>, right: Box<AtomExp>},
}
#[pymethods]
impl Exp {
    fn __str__(&self) -> String {
    ...
    pub fn get_type(&self) -> String {
        match self {
            ...
            Exp::BinaryExp {left: _, op, right: _} => match op {
                BinaryOp::Add => r!("Add"),
                BinaryOp::Sub => r!("Sub"),
                BinaryOp::Mul => r!("Mul"),
                BinaryOp::Div => r!("Div"),
            ...
            Exp::DiffExp {left: _, right: _, ord: _} => r!("DiffExp"),
            Exp::ExpWithMeasureType {exp: _, measuretype: _} => r!("ExpWithMeasureType"),
    ...
    pub fn from_atom(atom: &AtomExp) -> Self {
        Exp::Atom {atom: Box::new(atom.clone())}
    }
    #[staticmethod]
    ...
    pub fn get_all_partials(&self) -> HashSet<Exp> {
        match self {
            ...
            Exp::ExpWithMeasureType {exp, measuretype: _} => exp.get_all_partials(),
    ...
    pub fn subst(&self, oid: i32, nid: i32) -> Self{
        match self {
            ...
            Exp::Partial { left, right } => Exp::Partial { left: Box::new(left.subst(oid, nid)), right: Box::new(right.subst(oid, nid)) },
        }
    }
    ...
    #[inline]
    fn __neg__(&self) -> Self {
        (Exp::UnaryExp {op: UnaryOp::Neg, exp: Box::new(self.clone())}).doit()
    }
    #[inline]
    pub fn __difft__(&self, ord: i32) -> Self {
        (Exp::DiffExp {left: Box::new(self.clone()), right: Box::new(Exp::get_t()), ord}).doit()
    }
    #[inline]
    pub fn __diff__(&self, other: &Self) -> Self {
        (Exp::DiffExp {left: Box::new(self.clone()), right: Box::new(other.clone()), ord: 1}).doit()
    }
    #[inline]
    pub fn __partial__(&self, other: &AtomExp) -> Self {
        (Exp::Partial {left: Box::new(self.clone()), right: Box::new(other.clone())}).doit()
    }
    #[inline]
    fn replace_atom_by_exp(&self, atom: &AtomExp, exp: &Exp) -> Self {
        match self {
            ...
            Exp::UnaryExp { op, exp: e } => {
                Exp::UnaryExp {op: op.clone(), exp: Box::new(e.replace_atom_by_exp(atom, exp))}
            },
            Exp::BinaryExp { left: l, op, right: r } => {
                Exp::BinaryExp {
            ...
            Exp::DiffExp { left: l, right: r, ord } => {
                Exp::DiffExp {
                    left: Box::new(l.replace_atom_by_exp(atom, exp)),
                    right: Box::new(r.replace_atom_by_exp(atom, exp)),
                    ord: *ord
            ...
            Exp::ExpWithMeasureType { exp: e, measuretype: m } => {
    ...
    pub fn doit(&self) -> Exp {
        ...
        match self {
                    ...
                    UnaryOp::Diff => {
                        ...
                        match exp {
                            Exp::DiffExp {left, right, ord} => {
                                Exp::DiffExp {left: right, right: left, ord}
                            },
            ...
            Exp::DiffExp { left: l1, right: r1, ord } => {
                let left = l1.doit();
                let right = r1.doit();
                match left {
                    Exp::Number {num: _} => Exp::Number { num: 0 },
                    Exp::DiffExp { left: l2, right: r2, ord: o2 } => {
                        if &*r2 == &right {
                            Exp::DiffExp { left: l2, right: r2, ord: o2 + ord }
                        } else {
                            Exp::DiffExp { left: Box::new(
                                Exp::DiffExp { left: l2, right: r2, ord: o2 }
                            ), right: Box::new(right), ord: *ord }
                        }
                    },
                    Exp::UnaryExp { op: UnaryOp::Neg, exp } => {
                        Exp::UnaryExp { op: UnaryOp::Neg, exp: Box::new(
                            Exp::DiffExp { left: exp, right: Box::new(right), ord: *ord }
                        )}
                    }
                    _ => Exp::DiffExp { left: Box::new(left), right: Box::new(right), ord: *ord }
                }
            },
            Exp::ExpWithMeasureType { exp, measuretype } => {
                Exp::ExpWithMeasureType {exp: Box::new(exp.doit()), measuretype: measuretype.clone()}
            },
            Exp::Partial { left, right } => {
                let left = left.doit();
                match &left {
                    ...
                    Exp::UnaryExp { op, exp } => match op {
                        UnaryOp::Neg => -Exp::Partial {left: exp.clone(), right: right.clone()},
                        _ => Exp::Partial {left: Box::new(left), right: right.clone()}
                    },
                    _ => Exp::Partial { left: Box::new(left), right: right.clone() }
                }
            },
...
impl Ord for Exp {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        ...
        if ord { std::cmp::Ordering::Less } else { std::cmp::Ordering::Greater }
    }
}
...
impl SExp {
    ...
    fn __neg__(&self) -> Self {
        match self {
            SExp::Mk {expconfig, exp} => SExp::Mk {expconfig: expconfig.clone(), exp: Box::new(-*exp.clone())},
        }
    }
    fn __inv__(&self) -> Self {
        match self {
            SExp::Mk {expconfig, exp} => SExp::Mk {expconfig: expconfig.clone(), exp: Box::new(exp.clone().__powi__(-1))},
        }
    }
    #[new]#[inline]
...
pub enum Concept {
    ...
    // `[Sum: {objtype}] {concept}`
    Mksum {objtype: String, concept: Box<Concept>},
}
impl Concept {
    #[inline]
    ...
    fn _partial_subst(&self, idlist: Vec<i32>, summed_objtype: HashSet<String>, sub_dict: HashMap<i32, i32>) -> Result<Concept, String> {
        match self {
            Concept::Mk0 { exp } => {
                if idlist.len() > 0 {
                    return Err(r!("Error: idlist is to long for Concept::partial_subst function"))
                }
                Ok(Concept::Mk0 { exp: Box::new(exp.substs(&sub_dict)) })
            },
            Concept::Mksucc { objtype, concept, id } => {
                if summed_objtype.contains(objtype) {
                    ...
                    if jdlist.len() == 0 {
                        Ok(Concept::Mksucc {
                            objtype: objtype.clone(),
                            concept: Box::new(concept._partial_subst(jdlist, summed_objtype, sub_dict)?),
                            id: *id,
                        })
                    } else {
    ...
    fn _subst(&self, idlist: Vec<i32>, sub_dict: HashMap<i32, i32>) -> Result<Exp, String> {
        match self {
            ...
            Concept::Mksum { objtype: _, concept: _ } => {
    ...
    pub fn substs(&self, sub_dict: &HashMap<i32, i32>) -> Result<Exp, String> {
        match self {
            ...
            Concept::Mksum { objtype:_, concept:_ } => Err(r!("Error: Concept::Mksum cannot be substed.")),
        }
    }
...
impl Concept {
    ...
    pub fn is_sum(&self) -> bool {
        match self {
            Concept::Mksum {objtype: _, concept: _} => true,
            _ => false,
        }
    }
    ...
    pub fn get_exp(&self) -> Exp {
        match self {
            ...
            Concept::Mksum {objtype: _, concept } => concept.get_exp(),
        }
    }
    #[getter]#[inline]
...
impl IExpConfig {
    ...
    fn get_preobjtypes(&self) -> Vec<ObjType> {
        match self {
            IExpConfig::From {name: _} => vec![],
            IExpConfig::Mk {objtype, expconfig, id: _} => {
...
impl Intrinsic {
    ...
    fn __neg__(&self) -> Self {
        Intrinsic::From {sexp: Box::new(self.get_sexp().__neg__())}
    }
    fn __inv__(&self) -> Self {
        Intrinsic::From {sexp: Box::new(self.get_sexp().__inv__())}
    }
    #[new]#[inline]
    pub fn from_string(str: String) -> PyResult<Self> {
        Self::from_str(&str).map_err(|e| PyValueError::new_err(e))
