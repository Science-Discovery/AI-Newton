use super::{AtomExp, BinaryOp, Concept, Exp, Proposition};
...
impl Complexity<i32> for Proposition {
    fn complexity(&self) -> i32 {
        match self {
            Proposition::Conserved { concept } => concept.complexity() + 20,
            Proposition::Zero { concept } => concept.complexity(),
            Proposition::IsConserved { exp } => exp.complexity() + 20,
            Proposition::IsZero { exp } => exp.complexity(),
            Proposition::Eq { left, right } => left.complexity() + right.complexity(),
            Proposition::Not { prop } => prop.complexity(),
        }
    }
}
