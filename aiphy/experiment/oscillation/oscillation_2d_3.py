...
exp_para = {
    "x1": default_parastructure(-40.0, -35.0),
    "v1": default_parastructure(-2.0, 2.0),
    "x2": default_parastructure(-10.0, -5.0),
    "v2": default_parastructure(-2.0, 2.0),
    "x3": default_parastructure(5.0, 10.0),
    "v3": default_parastructure(-2.0, 2.0),
    "x4": default_parastructure(35.0, 40.0),
    "v4": default_parastructure(-2.0, 2.0),
...
obj_info = {
    ...
    "o4": Objstructure.make_particle(3.0, 6.0),
    "s1": Objstructure.make_spring(1.5, 2.5, 20.0, 30.0),
    "s2": Objstructure.make_spring(1.5, 2.5, 20.0, 30.0),
    "s3": Objstructure.make_spring(1.5, 2.5, 20.0, 30.0),
...
# eqs = [sp.sympify("a1*m1 + k1*(2*l1 + 2*x1 - 2*x2)/2"), sp.sympify("a2*m2 + k1*(-2*l1 - 2*x1 + 2*x2)/2 + k2*(2*l2 + 2*x2 - 2*x3)/2"), sp.sympify("a3*m3 + k2*(-2*l2 - 2*x2 + 2*x3)/2 + k3*(2*l3 + 2*x3 - 2*x4)/2"), sp.sympify("a4*m4 + k3*(-2*l3 - 2*x3 + 2*x4)/2")]
acs0 = [sp.sympify("k1*(-l1 - x1 + x2)/m1"), sp.sympify("(k1*l1 + k1*x1 - k1*x2 - k2*l2 - k2*x2 + k2*x3)/m2"), sp.sympify("(k2*l2 + k2*x2 - k2*x3 - k3*l3 - k3*x3 + k3*x4)/m3"), sp.sympify("k3*(l3 + x3 - x4)/m4")]
...
def do_experiment(t_end: float, t_num: int, error: float, exp_config: ExpConfig):
    ...
    v30 = exp_config.para('v3')
    x30 = exp_config.para('x3')
    m3 = exp_config.get_obj_para('o3', 'm')
    v40 = exp_config.para('v4')
    x40 = exp_config.para('x4')
    ...
    data_struct.insert_data((concept_posx, ['o3']), x3t*np.cos(theta) + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_posx, ['o4']), x4t*np.cos(theta) + np.random.normal(0, error, t_num))
    ...
    data_struct.insert_data((concept_posy, ['o3']), x3t*np.sin(theta) + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_posy, ['o4']), x4t*np.sin(theta) + np.random.normal(0, error, t_num))
    ...
    data_struct.insert_data((concept_length, ['s2']), x3t - x2t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_length, ['s3']), x4t - x3t + np.random.normal(0, error, t_num))
    ...
    data_struct.insert_data((concept_dist, ['o1', 'o3']), x3t-x1t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o3', 'o1']), x3t-x1t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o3', 'o2']), x3t-x2t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o2', 'o3']), x3t-x2t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o1', 'o4']), x4t-x1t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o4', 'o1']), x4t-x1t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o4', 'o3']), x4t-x3t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o3', 'o4']), x4t-x3t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o4', 'o2']), x4t-x2t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o2', 'o4']), x4t-x2t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_t, ['clock']), t + np.random.normal(0, error, t_num))
    return data_struct


def oscillation_2d_3_config() -> ExpStructure:
    expconfig = ExpConfig("oscillation_2d_3", 2, exp_para, obj_info, data_info)
...
def oscillation_2d_3_test():
    expconfig = ExpConfig("oscillation_2d_3", 1, exp_para, obj_info, data_info)
