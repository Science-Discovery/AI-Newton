import numpy as np
import sympy as sp
from scipy.integrate import odeint
from aiphy.experiment import default_parastructure, ExpConfig, Objstructure, DoExpType, ExpStructure, Proposition
from aiphy.experiment import concept_posx, concept_posy, concept_posz, concept_t, concept_length, concept_dist
# from aiphy.experiment.oscillation.free_oscillation import create_eqs


exp_para = {
    "x1": default_parastructure(-8.0, -7.0),
    "v1": default_parastructure(-2.0, 2.0),
    "x2": default_parastructure(-2.0, -1.0),
    "v2": default_parastructure(-2.0, 2.0),
    "x3": default_parastructure(1.0, 2.0),
    "v3": default_parastructure(-2.0, 2.0),
    "x4": default_parastructure(7.0, 8.0),
    "v4": default_parastructure(-2.0, 2.0),
}
obj_info = {
    "o1": Objstructure.make_particle(3.0, 6.0),
    "o2": Objstructure.make_particle(3.0, 6.0),
    "o3": Objstructure.make_particle(3.0, 6.0),
    "o4": Objstructure.make_particle(3.0, 6.0),
    "s1": Objstructure.make_spring(1.5, 2.5, 4.0, 6.0),
    "s2": Objstructure.make_spring(1.5, 2.5, 4.0, 6.0),
    "s3": Objstructure.make_spring(1.5, 2.5, 4.0, 6.0),
    "clock": Objstructure.clock()
}
data_info = [
    (concept_posx, ["o1"]),
    (concept_posx, ["o2"]),
    (concept_posx, ["o3"]),
    (concept_posx, ["o4"]),
    (concept_posy, ["o1"]),
    (concept_posy, ["o2"]),
    (concept_posy, ["o3"]),
    (concept_posy, ["o4"]),
    (concept_posz, ["o1"]),
    (concept_posz, ["o2"]),
    (concept_posz, ["o3"]),
    (concept_posz, ["o4"]),
    (concept_dist, ["o1", "o2"]),
    (concept_dist, ["o2", "o1"]),
    (concept_dist, ["o1", "o3"]),
    (concept_dist, ["o3", "o1"]),
    (concept_dist, ["o1", "o4"]),
    (concept_dist, ["o4", "o1"]),
    (concept_dist, ["o2", "o3"]),
    (concept_dist, ["o3", "o2"]),
    (concept_dist, ["o2", "o4"]),
    (concept_dist, ["o4", "o2"]),
    (concept_dist, ["o3", "o4"]),
    (concept_dist, ["o4", "o3"]),
    (concept_length, ["s1"]),
    (concept_length, ["s2"]),
    (concept_length, ["s3"]),
    (concept_t, ["clock"]),
]


# eqs = create_eqs(n=1+2)
# acs0 = [sp.symbols('a'+str(i+1)) for i in range(len(eqs))]
# sol = sp.solve(eqs, acs0)
# acs0 = [sp.simplify(ac.subs(sol)) for ac in acs0]
# print('eqs = [' + ', '.join(['sp.sympify("' + str(eq) + '")' for eq in eqs]) + ']')
# print('acs0 = [' + ', '.join(['sp.sympify("' + str(ac) + '")' for ac in acs0]) + ']')

# eqs = [sp.sympify("a1*m1 + k1*(2*l1 + 2*x1 - 2*x2)/2"), sp.sympify("a2*m2 + k1*(-2*l1 - 2*x1 + 2*x2)/2 + k2*(2*l2 + 2*x2 - 2*x3)/2"), sp.sympify("a3*m3 + k2*(-2*l2 - 2*x2 + 2*x3)/2")]
acs0 = [sp.sympify("k1*(-l1 - x1 + x2)/m1"), sp.sympify("(k1*l1 + k1*x1 - k1*x2 - k2*l2 - k2*x2 + k2*x3)/m2"), sp.sympify("(k2*l2 + k2*x2 - k2*x3 - k3*l3 - k3*x3 + k3*x4)/m3"), sp.sympify("k3*(l3 + x3 - x4)/m4")]


def do_experiment(t_end: float, t_num: int, error: float, exp_config: ExpConfig):
    v10 = exp_config.para('v1')
    x10 = exp_config.para('x1')
    m1 = exp_config.get_obj_para('o1', 'm')
    v20 = exp_config.para('v2')
    x20 = exp_config.para('x2')
    m2 = exp_config.get_obj_para('o2', 'm')
    v30 = exp_config.para('v3')
    x30 = exp_config.para('x3')
    m3 = exp_config.get_obj_para('o3', 'm')
    v40 = exp_config.para('v4')
    x40 = exp_config.para('x4')
    m4 = exp_config.get_obj_para('o4', 'm')
    k1 = exp_config.get_obj_para('s1', 'thickness')**3
    l1 = exp_config.get_obj_para('s1', 'freel')
    k2 = exp_config.get_obj_para('s2', 'thickness')**3
    l2 = exp_config.get_obj_para('s2', 'freel')
    k3 = exp_config.get_obj_para('s3', 'thickness')**3
    l3 = exp_config.get_obj_para('s3', 'freel')
    numeric = {'k1': k1, 'l1': l1, 'k2': k2, 'l2': l2, 'k3': k3, 'l3': l3,
               'm1': m1, 'm2': m2, 'm3': m3, 'm4': m4}
    acs = [ac.subs(numeric) for ac in acs0]

    step = t_end / t_num
    t = np.arange(0, t_end, step)
    y0 = [x10, x20, x30, x40, v10, v20, v30, v40]

    def f(y, t):
        x1, x2, x3, x4, v1, v2, v3, v4 = y
        numeric = {'x1': x1, 'v1': v1, 'x2': x2, 'v2': v2,
                   'x3': x3, 'v3': v3, 'x4': x4, 'v4': v4,
                   }
        acs_n = [ac.subs(numeric) for ac in acs]
        return [v1, v2, v3, v4] + acs_n

    sol = odeint(f, y0, t)

    x1t = sol[:, 0]
    x2t = sol[:, 1]
    x3t = sol[:, 2]
    x4t = sol[:, 3]

    data_struct = exp_config.new_datastruct_of_doexperiment(t_num)
    data_struct.insert_data((concept_posx, ['o1']), x1t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_posx, ['o2']), x2t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_posx, ['o3']), x3t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_posx, ['o4']), x4t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_posy, ['o1']), np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_posy, ['o2']), np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_posy, ['o3']), np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_posy, ['o4']), np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_posz, ['o1']), np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_posz, ['o2']), np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_posz, ['o3']), np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_posz, ['o4']), np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o1', 'o2']), x2t-x1t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o2', 'o1']), x2t-x1t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o1', 'o3']), x3t-x1t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o3', 'o1']), x3t-x1t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o1', 'o4']), x4t-x1t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o4', 'o1']), x4t-x1t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o2', 'o3']), x3t-x2t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o3', 'o2']), x3t-x2t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o2', 'o4']), x4t-x2t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o4', 'o2']), x4t-x2t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o3', 'o4']), x4t-x3t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_dist, ['o4', 'o3']), x4t-x3t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_length, ['s1']), x2t-x1t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_length, ['s2']), x3t-x2t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_length, ['s3']), x4t - x3t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_t, ['clock']), t + np.random.normal(0, error, t_num))
    return data_struct


def oscillation_x_3_config() -> ExpStructure:
    expconfig = ExpConfig("oscillation_x_3", 1, exp_para, obj_info, data_info)
    doexp = DoExpType(__file__)

    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posx[o2] - posx[o1] - length[s1]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posx[o3] - posx[o2] - length[s2]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posx[o4] - posx[o3] - length[s3]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posx[o2] - posx[o1] - dist[o1, o2]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "dist[o1, o2] - dist[o2, o1]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posx[o3] - posx[o1] - dist[o1, o3]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "dist[o1, o3] - dist[o3, o1]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posx[o4] - posx[o1] - dist[o1, o4]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "dist[o1, o4] - dist[o4, o1]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posx[o3] - posx[o2] - dist[o2, o3]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "dist[o2, o3] - dist[o3, o2]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posx[o4] - posx[o2] - dist[o2, o4]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "dist[o2, o4] - dist[o4, o2]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posx[o4] - posx[o3] - dist[o3, o4]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "dist[o3, o4] - dist[o4, o3]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posy[o1]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posy[o2]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posy[o3]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posy[o4]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posz[o1]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posz[o2]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posz[o3]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posz[o4]")))

    expstructure = ExpStructure(expconfig, doexp)
    return expstructure


def oscillation_x_3_test():
    expconfig = ExpConfig("oscillation_x_3", 1, exp_para, obj_info, data_info)
    expconfig.random_settings()
    do_experiment(10, 100, 0.1, expconfig)
