...
exp_para = {
    ...
    "posl2": default_parastructure(-0.5, 0.5),
}
obj_info = {
...
acs0 = [sp.sympify("k1*k2*(2*posr1+2*posr2-z1-2*z2-z3-l0-2*l1-2*l2)/4/(k1+k2)/m1 - g"),
        sp.sympify("k1*k2*(2*posr1+2*posr2-z1-2*z2-z3-l0-2*l1-2*l2)/2/(k1+k2)/m2 - g"),
        sp.sympify("k1*k2*(2*posr1+2*posr2-z1-2*z2-z3-l0-2*l1-2*l2)/4/(k1+k2)/m3 - g")]
...
def do_experiment(t_end: float, t_num: int, error: float, exp_config: ExpConfig):
    ...
    l0 = 2*posl1+2*posl2-z10-2*z20-z30
    step = t_end / t_num
    t = np.arange(0, t_end, step)
    g = 9.801234567

    numeric = {'k1': k1, 'l1': l1, 'posr1': posr1,
               'k2': k2, 'l2': l2, 'posr2': posr2,
               'm1': m1, 'm2': m2, 'm3': m3,
               'l0': l0, 'g': g}
    acs = [ac.subs(numeric) for ac in acs0]
    input0 = [z10, z20, z30, 0, 0, 0]

    def f(y, t):
        z1, z2, z3, v1, v2, v3 = y
        numeric = {'z1': z1, 'z2': z2, 'z3': z3}
    ...
    z2t = sol[:, 1]
    z3t = sol[:, 2]
    len1t = (2*k1*l1-k2*(l0+2*l2-2*posr1-2*posr2+z1t+2*z2t+z3t))/(2*(k1+k2))
    len2t = (2*k2*l2-k1*(l0+2*l1-2*posr1-2*posr2+z1t+2*z2t+z3t))/(2*(k1+k2))
...
def pulley_oscillation_2_config() -> ExpStructure:
    expconfig = ExpConfig("pulley_oscillation_2", 2, exp_para, obj_info, data_info)
    doexp = DoExpType(__file__)

    expconfig.register_geometry_info(Proposition.IsConserved(expconfig.gen_exp(
        "posz[o1] + 2*posz[o2] + posz[o3] + 2*length[s1] + 2*length[s2]")))
