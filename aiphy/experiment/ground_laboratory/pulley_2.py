...
def do_experiment(t_end: float, t_num: int, error: float, exp_config: ExpConfig):
    ...
    a1 = g * (-m1*m2-4*m1*m3+3*m2*m3)/(m1*m2+4*m1*m3+m2*m3)
    a2 = g * (-m1*m2+4*m1*m3 - m2*m3)/(m1*m2+4*m1*m3+m2*m3)
    a3 = g * (-m2*m3-4*m1*m3+3*m1*m2)/(m1*m2+4*m1*m3+m2*m3)
    z1t = z1 + a1*t**2 / 2
    z2t = z2 + a2*t**2 / 2
    z3t = z3 + a3*t**2 / 2
...
def pulley_2_config() -> ExpStructure:
    expconfig = ExpConfig("pulley_2", 2, exp_para, obj_info, data_info)
    doexp = DoExpType(__file__)

    expconfig.register_geometry_info(Proposition.IsConserved(expconfig.gen_exp(
        "posz[o1] + 2*posz[o2] + posz[o3]")))
