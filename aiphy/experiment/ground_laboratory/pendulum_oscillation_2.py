...
exp_para = {
    ...
    'omega2': default_parastructure(-pi/3, pi/3),
}
obj_info = {
...
g = 9.801234567
# eqs = create_eqs(n=2)
# acs0 = [sp.symbols('a'+str(i+1)) for i in range(int(len(eqs))//2)] + [sp.symbols('beta'+str(i+1)) for i in range(int(len(eqs))//2)]
# sol = sp.solve(eqs, acs0)
# acs0 = [sp.simplify(ac.subs(sol)) for ac in acs0]
# print('eqs = [' + ', '.join(['sp.sympify("' + str(eq) + '")' for eq in eqs]) + ']')
# print('acs0 = [' + ', '.join(['sp.sympify("' + str(acs) + '")' for acs in acs0]) + ']')
acs0 = [
    sp.sympify("g*cos(theta1) + omega1**2*r1 + (k1*(l1-r1)+k2*cos(theta1 - theta2)*(r2-l2))/m1"),
    sp.sympify("omega2**2*r2 + k1*cos(theta1 - theta2)*(r1-l1)/m1 + k2*(m1+m2)*(l2-r2)/(m1*m2)"),
    sp.sympify("-(g*sin(theta1) + 2*omega1*v1)/r1 + k2*(l2-r2)*sin(theta1 - theta2)/(r1*m1)"),
...
def do_experiment(t_end: float, t_num: int, error: float, exp_config: ExpConfig):
    ...
    numeric = {'g': g, 'm1': m1, 'l1': l1, 'k1': k1, 'm2': m2, 'l2': l2, 'k2': k2}
    ...
    posx2 = posx1 + r2*np.sin(theta2)
    posz2 = posz1 - r2*np.cos(theta2)
...
def pendulum_oscillation_2_config() -> ExpStructure:
    expconfig = ExpConfig("pendulum_oscillation_2", 2, exp_para, obj_info, data_info)
    ...
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "(posz[o1]-posz[o2])**2 + (posx[o1]-posx[o2])**2 - length[s2]**2")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posy[o1]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posy[o2]")))
...
def pendulum_oscillation_2_test():
    expconfig = ExpConfig("pendulum_oscillation_2", 2, exp_para, obj_info, data_info)
