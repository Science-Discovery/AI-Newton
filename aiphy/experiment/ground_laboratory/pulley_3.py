...
def do_experiment(t_end: float, t_num: int, error: float, exp_config: ExpConfig):
    ...
    z3 = exp_config.para('z3')
    z4 = exp_config.para('z4')
    ...
    a1 = g * (-m1*m2*m3-4*m1*m2*m4-4*m1*m3*m4+5*m2*m3*m4)/(m1*m2*m3+4*m1*m2*m4+4*m1*m3*m4+m2*m3*m4)
    a2 = g * (-m1*m2*m3-4*m1*m2*m4+8*m1*m3*m4-m2*m3*m4)/(m1*m2*m3+4*m1*m2*m4+4*m1*m3*m4+m2*m3*m4)
    a3 = g * (-m1*m2*m3+8*m1*m2*m4-4*m1*m3*m4-m2*m3*m4)/(m1*m2*m3+4*m1*m2*m4+4*m1*m3*m4+m2*m3*m4)
    a4 = g * (5*m1*m2*m3-4*m1*m2*m4-4*m1*m3*m4-m2*m3*m4)/(m1*m2*m3+4*m1*m2*m4+4*m1*m3*m4+m2*m3*m4)
    z1t = z1 + a1*t**2 / 2
    z2t = z2 + a2*t**2 / 2
    z3t = z3 + a3*t**2 / 2
    z4t = z4 + a4*t**2 / 2
    ...
    data_struct.insert_data((concept_posx, ['o4']), np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_posy, ['o4']), np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_posz, ['o4']), z4t + np.random.normal(0, error, t_num))
    data_struct.insert_data((concept_t, ['clock']), t + np.random.normal(0, error, t_num))
    return data_struct

def pulley_3_config() -> ExpStructure:
    expconfig = ExpConfig("pulley_3", 2, exp_para, obj_info, data_info)
    doexp = DoExpType(__file__)

    expconfig.register_geometry_info(Proposition.IsConserved(expconfig.gen_exp(
        "posz[o1] + 2*posz[o2] + 2*posz[o3] + posz[o4]")))
