...
G = 6.6e-11
acs0 = [sp.sympify("G*m2*(z2-z1)/r12**3"),
...
def do_experiment(t_end: float, t_num: int, error: float, exp_config: ExpConfig):
    ...
    z10 = z0 + r0*np.cos(theta0)
    z20 = z0 - r0*np.cos(theta0)
    y10 = y0 + r0*np.sin(theta0)
    y20 = y0 - r0*np.sin(theta0)
    vz10 = v10*np.cos(theta0) - omega10*r0*np.sin(theta0)
    vy10 = v10*np.sin(theta0) + omega10*r0*np.cos(theta0)
    vz20 = v20*np.cos(theta0) + omega20*r0*np.sin(theta0)
    ...
    y0 = [z10, z20, y10, y20, vz10, vz20, vy10, vy20]

    def f(y, t):
        z1, z2, y1, y2, vz1, vz2, vy1, vy2 = y
        r12 = np.sqrt((z1-z2)**2 + (y1-y2)**2)
        numeric = {'z1': z1, 'z2': z2, 'y1': y1, 'y2': y2, 'r12': r12,
                   'vz1': vz1, 'vz2': vz2, 'vy1': vy1, 'vy2': vy2}
        acs_n = [ac.subs(numeric).evalf() for ac in acs]
        return [vz1, vz2, vy1, vy2] + acs_n
    sol = odeint(f, y0, t)
    z1 = sol[:, 0]
    z2 = sol[:, 1]
    y1 = sol[:, 2]
    y2 = sol[:, 3]
    dist = np.sqrt((z1-z2)**2 + (y1-y2)**2)
...
def celestial_2_yz_config() -> ExpStructure:
    expconfig = ExpConfig("celestial_2_yz", 2, exp_para, obj_info, data_info)
    doexp = DoExpType(__file__)

    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "dist[o1, o2] - dist[o2, o1]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "dist[o1, o2] ** 2 - (posz[o1] - posz[o2]) ** 2 - (posy[o1] - posy[o2]) ** 2")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posx[o1]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posx[o2]")))
