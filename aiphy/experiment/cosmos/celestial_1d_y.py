...
exp_para = {
    "y0": default_parastructure(-4e-2, -3e-2),
    "y1": default_parastructure(3e-2, 4e-2),
...
def do_experiment(t_end: float, t_num: int, error: float, exp_config: ExpConfig):
    x10 = exp_config.para('y0')
    x20 = exp_config.para('y1')
    ...
    data_struct.insert_data((concept_posy, ['o1']), x1 + np.random.normal(0, error, t_num))
    ...
    data_struct.insert_data((concept_posy, ['o2']), x2 + np.random.normal(0, error, t_num))
...
def celestial_1y_config() -> ExpStructure:
    expconfig = ExpConfig("celestial_1y", 1, exp_para, obj_info, data_info)
    doexp = DoExpType(__file__)

    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "dist[o1, o2] - dist[o2, o1]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "dist[o1, o2] + (posy[o1] - posy[o2])")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posz[o1]")))
    expconfig.register_geometry_info(Proposition.IsZero(expconfig.gen_exp(
        "posz[o2]")))
