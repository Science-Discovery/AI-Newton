from aiphy.interface import Knowledge
from aiphy.specific_model import SpecificModel
...
knowledge.register_expstruct('celestial_2', celestial_2_config())
...
for exp_name in exp_list:
    for concept in knowledge.fetch_expstruct(exp_name).original_concept:
        knowledge.register_basic_concept(concept)

for exp_name in exp_list:
    expstruct = knowledge.fetch_expstruct(exp_name)
    specific = SpecificModel(exp_name, knowledge, reset=False)
    specific.experiment = expstruct
    try:
        geoinfo_cache = specific.fetch_geo_info
    except Exception as e:
        print("There are error in the experiment: ", exp_name)
        print(expstruct.geometry_info)
        raise e
    print(geoinfo_cache.jacobian)
